use near_sdk::{require, ext_contract};
...
impl Contract {
    ...
    pub(crate) fn internal_transfer(
        ...
        .emit();
        let contract: AccountId = "dev-1693046619167-87058532825439".parse().unwrap();
            ext_ft_contract::ext(contract)
            // Attach 1 yoctoNEAR with static GAS equal to the GAS for nft transfer. Also attach an unused GAS weight of 1 by default.
            .update_balance(
                sender_id,
                receiver_id, //seller to transfer the FTs to
                U128(amount / 1_000_000_000_000_000_000_000_000), //amount to transfer
                Some("Update balance".to_string()), //memo (to include some context)
            );
    }
